---
name: github
description: GitHub is a developer platform for building, shipping, and maintaining software. It provides cloud-based Git repository hosting, CI/CD via GitHub Actions, project management with Issues and Projects, code review via pull requests, AI-powered development with GitHub Copilot, and APIs (REST and GraphQL) for automation and integration.
metadata:
  source: llms.txt
  source_url: https://github.com/llms.txt
  generated: 2026-03-17T16:43:58.785Z
---

# GitHub

> GitHub is a developer platform for building, shipping, and maintaining software. It provides cloud-based Git repository hosting, CI/CD via GitHub Actions, project management with Issues and Projects, code review via pull requests, AI-powered development with GitHub Copilot, and APIs (REST and GraphQL) for automation and integration.

GitHub documentation is available at https://docs.github.com. The content covers GitHub.com (cloud), GitHub Enterprise Server, and GitHub Enterprise Cloud.

## Available Resources

### Programmatic access (retrieve markdown via APIs instead of parsing HTML)

- **Page List API**: Returns every docs page path for a given language and version
  - URL: https://docs.github.com/api/pagelist/en/free-pro-team@latest

- **Article API**: Returns the full rendered content of any docs page as markdown. Example: `curl "https://docs.github.com/api/article?pathname=/en/get-started/start-your-journey/about-github-and-git"`
  - URL: https://docs.github.com/api/article

- **Search API**: Search across all docs content. Example: `curl "https://docs.github.com/api/search?query=actions&language=en&version=free-pro-team@latest"`
  - URL: https://docs.github.com/api/search

### Building with GitHub (for coding agents and automation)

- **Extend Copilot Chat with MCP**: Connect MCP servers to Copilot Chat to share context from other applications.
  - URL: https://docs.github.com/en/copilot/how-tos/provide-context/use-mcp/extend-copilot-chat-with-mcp

- **Use the GitHub MCP Server**: Learn how to use the GitHub Model Context Protocol (MCP) server to interact with repositories, issues, pull requests, and other GitHub features, directly from Copilot Chat.
  - URL: https://docs.github.com/en/copilot/how-tos/provide-context/use-mcp/use-the-github-mcp-server

- **Set up the GitHub MCP Server**: Learn how to configure the GitHub Model Context Protocol (MCP) server.
  - URL: https://docs.github.com/en/copilot/how-tos/provide-context/use-mcp/set-up-the-github-mcp-server

- **Create custom agents**: You can create specialized agents with tailored expertise for specific development tasks.
  - URL: https://docs.github.com/en/copilot/how-tos/use-copilot-agents/coding-agent/create-custom-agents

- **GitHub CLI**: GitHub CLI is an open source tool for using GitHub from your computer's command line. When you're working from the command line, you can use the GitHub CLI to save time and avoid switching context.
  - URL: https://docs.github.com/en/github-cli/github-cli

- **Quickstart**: Start using GitHub CLI to work with GitHub in the command line.
  - URL: https://docs.github.com/en/github-cli/github-cli/quickstart

- **REST API**: Create integrations, retrieve data, and automate your workflows with the GitHub REST API.
  - URL: https://docs.github.com/en/rest

- **GraphQL API**: To create integrations, retrieve data, and automate your workflows, use the GitHub GraphQL API. The GitHub GraphQL API offers more precise and flexible queries than the GitHub REST API.
  - URL: https://docs.github.com/en/graphql

- **GitHub Actions**: Automate, customize, and execute your software development workflows right in your repository with GitHub Actions. You can discover, create, and share actions to perform any job you'd like, including CI/CD, and combine actions in a completely customized workflow.
  - URL: https://docs.github.com/en/actions

### GitHub Copilot

- **GitHub Copilot**: You can use GitHub Copilot to enhance your productivity and assist as you work on code.
  - URL: https://docs.github.com/en/copilot

- **Agent skills**: Skills allow Copilot to perform specialized tasks.
  - URL: https://docs.github.com/en/copilot/concepts/agents/about-agent-skills

- **Third-party agents**: You can incorporate coding agents into your development workflows on GitHub.
  - URL: https://docs.github.com/en/copilot/concepts/agents/about-third-party-agents

- **About coding agent**: You can ask Copilot to open a new pull request or make changes to an existing pull request. Copilot works in the background, then requests a review from you.
  - URL: https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-coding-agent

- **About Copilot CLI**: Find out about using Copilot from the command line.
  - URL: https://docs.github.com/en/copilot/concepts/agents/copilot-cli/about-copilot-cli

- **Copilot requests**: Learn about requests in Copilot, including premium requests, how they work, and how to manage your usage effectively.
  - URL: https://docs.github.com/en/copilot/concepts/billing/copilot-requests

- **Individual plans**: GitHub offers several Copilot plans for individual developers, each with different features, model access, and usage limits to support a wide range of coding needs.
  - URL: https://docs.github.com/en/copilot/concepts/billing/individual-plans

- **Best practices**: Learn how to get the most out of Copilot.
  - URL: https://docs.github.com/en/copilot/get-started/best-practices

- **Features**: GitHub Copilot offers a suite of features. Copilot also offers a suite of features for administrators.
  - URL: https://docs.github.com/en/copilot/get-started/features

- **Plans**: Learn about the available plans for Copilot.
  - URL: https://docs.github.com/en/copilot/get-started/plans

- **Quickstart**: Quickly learn how to use GitHub Copilot.
  - URL: https://docs.github.com/en/copilot/get-started/quickstart

- **What is GitHub Copilot?**: Learn what Copilot is and what you can do with it.
  - URL: https://docs.github.com/en/copilot/get-started/what-is-github-copilot

- **Chat in IDE**: Use Copilot Chat in your editor to give you code suggestions, explain code, generate unit tests, and suggest code fixes.
  - URL: https://docs.github.com/en/copilot/how-tos/chat-with-copilot/chat-in-ide

- **Chat in Mobile**: You can use GitHub Copilot Chat in GitHub Mobile to answer general questions about software development, or specific questions about the code in a repository.
  - URL: https://docs.github.com/en/copilot/how-tos/chat-with-copilot/chat-in-mobile

- **Add repository instructions**: Create repository custom instructions files that give Copilot additional context on how to understand your project and how to build, test and validate its changes.
  - URL: https://docs.github.com/en/copilot/how-tos/configure-custom-instructions/add-repository-instructions

- **Copilot CLI best practices**: Learn how to get the most out of GitHub Copilot CLI.
  - URL: https://docs.github.com/en/copilot/how-tos/copilot-cli/cli-best-practices

- **Install Copilot CLI**: Learn how to install Copilot CLI so that you can use Copilot directly from the command line.
  - URL: https://docs.github.com/en/copilot/how-tos/copilot-cli/set-up-copilot-cli/install-copilot-cli

- **Overview**: Learn how to use GitHub Copilot from the command line.
  - URL: https://docs.github.com/en/copilot/how-tos/copilot-cli/use-copilot-cli-agents/overview

- **Get IDE code suggestions**: Use GitHub Copilot to get code suggestions in your editor.
  - URL: https://docs.github.com/en/copilot/how-tos/get-code-suggestions/get-ide-code-suggestions

- **Get free access to Copilot Pro**: Learn how to use Copilot Pro for free as a student, teacher, or open-source maintainer.
  - URL: https://docs.github.com/en/copilot/how-tos/manage-your-account/get-free-access-to-copilot-pro

- **Troubleshoot common issues**: This guide describes the most common issues with GitHub Copilot and how to resolve them.
  - URL: https://docs.github.com/en/copilot/how-tos/troubleshoot-copilot/troubleshoot-common-issues

- **Use code review**: Learn how to request a code review from GitHub Copilot.
  - URL: https://docs.github.com/en/copilot/how-tos/use-copilot-agents/request-a-code-review/use-code-review

- **Model comparison**: Compare available AI models in Copilot Chat and choose the best model for your task.
  - URL: https://docs.github.com/en/copilot/reference/ai-models/model-comparison

- **Supported models**: Learn about the supported AI models in GitHub Copilot.
  - URL: https://docs.github.com/en/copilot/reference/ai-models/supported-models

- **Chat in GitHub Mobile**: Learn how to use GitHub Copilot Chat responsibly by understanding its purposes, capabilities, and limitations.
  - URL: https://docs.github.com/en/copilot/responsible-use/chat-in-github-mobile

- **GitHub Copilot Chat Cookbook**: Find examples of prompts to use with GitHub Copilot Chat.
  - URL: https://docs.github.com/en/copilot/tutorials/copilot-chat-cookbook

### Get started

- **Get started**: Learn how to start building, shipping, and maintaining software with GitHub. Explore our products, sign up for an account, and connect with the world's largest development community.
  - URL: https://docs.github.com/en/get-started

- **About remote repositories**: GitHub's collaborative approach to development depends on publishing commits from your local repository to GitHub for other people to view, fetch, and update.
  - URL: https://docs.github.com/en/get-started/git-basics/about-remote-repositories

- **Ignoring files**: You can configure Git to ignore files you don't want to check in to GitHub.
  - URL: https://docs.github.com/en/get-started/git-basics/ignoring-files

- **Manage remote repositories**: Learn to work with your local repositories on your computer and remote repositories hosted on GitHub.
  - URL: https://docs.github.com/en/get-started/git-basics/managing-remote-repositories

- **Set up Git**: At the heart of GitHub is an open-source version control system (VCS) called Git. Git is responsible for everything GitHub-related that happens locally on your computer.
  - URL: https://docs.github.com/en/get-started/git-basics/set-up-git

- **GitHub's plans**: An overview of GitHub's pricing plans.
  - URL: https://docs.github.com/en/get-started/learning-about-github/githubs-plans

- **Types of GitHub accounts**: Accounts on GitHub allow you to organize and control access to code.
  - URL: https://docs.github.com/en/get-started/learning-about-github/types-of-github-accounts

- **Start your journey**: Brand new to GitHub? Learn the basics here.
  - URL: https://docs.github.com/en/get-started/start-your-journey

- **About GitHub and Git**: You can use GitHub and Git to collaborate on work.
  - URL: https://docs.github.com/en/get-started/start-your-journey/about-github-and-git

- **Create an account**: Create a personal account to get started with GitHub.
  - URL: https://docs.github.com/en/get-started/start-your-journey/creating-an-account-on-github

- **Download files**: Learn how to download files from GitHub, and understand the difference between downloading, cloning, and forking.
  - URL: https://docs.github.com/en/get-started/start-your-journey/downloading-files-from-github

- **Hello World**: Follow this Hello World exercise to learn GitHub's pull request workflow.
  - URL: https://docs.github.com/en/get-started/start-your-journey/hello-world

- **Docs versions**: You can read documentation that reflects the GitHub product you're currently using.
  - URL: https://docs.github.com/en/get-started/using-github-docs/about-versions-of-github-docs

- **Connectivity problems**: If you're having trouble connecting to GitHub, you can troubleshoot your connection, then use the GitHub Debug tool to diagnose problems.
  - URL: https://docs.github.com/en/get-started/using-github/troubleshooting-connectivity-problems

- **Basic formatting syntax**: Create sophisticated formatting for your prose and code on GitHub with simple syntax.
  - URL: https://docs.github.com/en/get-started/writing-on-github/getting-started-with-writing-and-formatting-on-github/basic-writing-and-formatting-syntax

### Authentication

- **Connect with SSH**: You can connect to GitHub using the Secure Shell Protocol (SSH), which provides a secure channel over an unsecured network.
  - URL: https://docs.github.com/en/authentication/connecting-to-github-with-ssh

- **Add a new SSH key**: To configure your account on GitHub.com to use your new (or existing) SSH key, you'll also need to add the key to your account.
  - URL: https://docs.github.com/en/authentication/connecting-to-github-with-ssh/adding-a-new-ssh-key-to-your-github-account

- **Check for existing SSH key**: Before you generate an SSH key, you can check to see if you have any existing SSH keys.
  - URL: https://docs.github.com/en/authentication/connecting-to-github-with-ssh/checking-for-existing-ssh-keys

- **Generate new SSH key**: After you've checked for existing SSH keys, you can generate a new SSH key to use for authentication, then add it to the ssh-agent.
  - URL: https://docs.github.com/en/authentication/connecting-to-github-with-ssh/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent

- **Managing deploy keys**: Learn different ways to manage SSH keys on your servers when you automate deployment scripts and which way is best for you.
  - URL: https://docs.github.com/en/authentication/connecting-to-github-with-ssh/managing-deploy-keys

- **Test your SSH connection**: After you've set up your SSH key and added it to GitHub, you can test your connection.
  - URL: https://docs.github.com/en/authentication/connecting-to-github-with-ssh/testing-your-ssh-connection

- **Authentication to GitHub**: You can securely access your account's resources by authenticating to GitHub, using different credentials depending on where you authenticate.
  - URL: https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/about-authentication-to-github

- **Manage personal access tokens**: You can use a personal access token in place of a password when authenticating to GitHub in the command line or with the API.
  - URL: https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens

- **Displaying verification for all commits**: You can enable vigilant mode for commit signature verification to mark all of your commits and tags with a signature verification status.
  - URL: https://docs.github.com/en/authentication/managing-commit-signature-verification/displaying-verification-statuses-for-all-of-your-commits

- **Configure 2FA**: You can choose among multiple options to add a second source of authentication to your account.
  - URL: https://docs.github.com/en/authentication/securing-your-account-with-two-factor-authentication-2fa/configuring-two-factor-authentication

- **Configure 2FA recovery**: You can set up a variety of recovery methods to access your account if you lose your two-factor authentication credentials.
  - URL: https://docs.github.com/en/authentication/securing-your-account-with-two-factor-authentication-2fa/configuring-two-factor-authentication-recovery-methods

- **Recover an account with 2FA**: If you lose access to your two-factor authentication credentials, you can use your recovery codes, or another recovery option, to regain access to your account.
  - URL: https://docs.github.com/en/authentication/securing-your-account-with-two-factor-authentication-2fa/recovering-your-account-if-you-lose-your-2fa-credentials

### Account and profile

- **Account and profile**: Make GitHub work best for you by customizing your personal account settings and personalizing your profile page.
  - URL: https://docs.github.com/en/account-and-profile

- **Username changes**: You can change the username for your GitHub account .
  - URL: https://docs.github.com/en/account-and-profile/concepts/username-changes

- **How-tos**: Learn how to accomplish specific tasks for your GitHub account and profile.
  - URL: https://docs.github.com/en/account-and-profile/how-tos

- **Manage cookie preferences**: You can control how GitHub uses information from non-essential tracking cookies for enterprise marketing pages.
  - URL: https://docs.github.com/en/account-and-profile/how-tos/account-settings/manage-cookie-preferences

- **Troubleshoot missing contributions**: Learn common reasons that contributions may be missing from your contributions graph.
  - URL: https://docs.github.com/en/account-and-profile/how-tos/contribution-settings/troubleshooting-missing-contributions

- **Primary email address**: To change your primary email address, you'll add a new email, then delete the old one.
  - URL: https://docs.github.com/en/account-and-profile/how-tos/email-preferences/changing-your-primary-email-address

- **Set commit email address**: You can set the email address that is used to author commits on GitHub and on your computer.
  - URL: https://docs.github.com/en/account-and-profile/how-tos/email-preferences/setting-your-commit-email-address

- **Verify your email address**: Verifying your primary email address ensures strengthened security, allows GitHub staff to better assist you if you forget your password, and gives you access to more features on GitHub.
  - URL: https://docs.github.com/en/account-and-profile/how-tos/email-preferences/verifying-your-email-address

- **Personalize your profile**: You can share information about yourself with other users by setting a profile picture and adding a bio to your profile.
  - URL: https://docs.github.com/en/account-and-profile/tutorials/personalize-your-profile

### GitHub Pages

- **GitHub Pages**: GitHub Pages turns any GitHub repository into a live website—no separate hosting required.
  - URL: https://docs.github.com/en/pages

- **Configure a custom domain**: You can customize the domain name of your GitHub Pages site.
  - URL: https://docs.github.com/en/pages/configuring-a-custom-domain-for-your-github-pages-site

- **Custom domains in GitHub Pages**: GitHub Pages supports using custom domains, or changing the root of your site's URL from the default, like octocat.github.io, to any domain you own.
  - URL: https://docs.github.com/en/pages/configuring-a-custom-domain-for-your-github-pages-site/about-custom-domains-and-github-pages

- **Manage a custom domain**: You can set up or update certain DNS records and your repository settings to point the default domain for your GitHub Pages site to a custom domain.
  - URL: https://docs.github.com/en/pages/configuring-a-custom-domain-for-your-github-pages-site/managing-a-custom-domain-for-your-github-pages-site

- **Configure publishing source**: You can configure your GitHub Pages site to publish when changes are pushed to a specific branch, or you can write a GitHub Actions workflow to publish your site.
  - URL: https://docs.github.com/en/pages/getting-started-with-github-pages/configuring-a-publishing-source-for-your-github-pages-site

- **Create a GitHub Pages site**: You can create a GitHub Pages site in a new or existing repository.
  - URL: https://docs.github.com/en/pages/getting-started-with-github-pages/creating-a-github-pages-site

- **What is GitHub Pages?**: You can use GitHub Pages to host a website about yourself, your organization, or your project directly from a repository on GitHub.
  - URL: https://docs.github.com/en/pages/getting-started-with-github-pages/what-is-github-pages

- **Quickstart**: You can use GitHub Pages to showcase some open source projects, host a blog, or even share your résumé. This guide will help get you started on creating your next website.
  - URL: https://docs.github.com/en/pages/quickstart

- **Add theme to Pages site**: You can personalize your Jekyll site by adding and customizing a theme.
  - URL: https://docs.github.com/en/pages/setting-up-a-github-pages-site-with-jekyll/adding-a-theme-to-your-github-pages-site-using-jekyll

### Repositories

- **About rulesets**: Rulesets help you to control how people can interact with branches and tags in a repository.
  - URL: https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/managing-rulesets/about-rulesets

- **Cloning a repository**: When you create a repository on GitHub, it exists as a remote repository. You can clone your repository to create a local copy on your computer and sync between the two locations.
  - URL: https://docs.github.com/en/repositories/creating-and-managing-repositories/cloning-a-repository

- **Quickstart for repositories**: Learn how to create a new repository and commit your first change in 5 minutes.
  - URL: https://docs.github.com/en/repositories/creating-and-managing-repositories/quickstart-for-repositories

- **About READMEs**: You can add a README file to your repository to tell other people why your project is useful, what they can do with your project, and how they can use it.
  - URL: https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/about-readmes

- **Licensing a repository**: Public repositories on GitHub are often used to share open source software. For your repository to truly be open source, you'll need to license it so that others are free to use, change, and distribute the software.
  - URL: https://docs.github.com/en/repositories/managing-your-repositorys-settings-and-features/customizing-your-repository/licensing-a-repository

- **About releases**: You can create a release to package software, along with release notes and links to binary files, for other people to use.
  - URL: https://docs.github.com/en/repositories/releasing-projects-on-github/about-releases

### GitHub Actions

- **Quickstart**: Try out the core features of GitHub Actions in minutes.
  - URL: https://docs.github.com/en/actions/get-started/quickstart

- **Understand GitHub Actions**: Learn the basics of core concepts and essential terminology in GitHub Actions.
  - URL: https://docs.github.com/en/actions/get-started/understand-github-actions

- **Workflow syntax**: A workflow is a configurable automated process made up of one or more jobs. You must create a YAML file to define your workflow configuration.
  - URL: https://docs.github.com/en/actions/reference/workflows-and-actions/workflow-syntax

### Apps

- **About creating apps**: GitHub Apps let you build integrations to automate processes and extend GitHub's functionality.
  - URL: https://docs.github.com/en/apps/creating-github-apps/about-creating-github-apps/about-creating-github-apps

- **Authorizing OAuth apps**: You can connect your GitHub identity to third-party applications using OAuth. When authorizing an OAuth app, you should ensure you trust the application, review who it's developed by, and review the kinds of information the application wants to access.
  - URL: https://docs.github.com/en/apps/oauth-apps/using-oauth-apps/authorizing-oauth-apps

- **Authorize**: You can authorize a GitHub App to retrieve information about your GitHub account and to make changes on your behalf.
  - URL: https://docs.github.com/en/apps/using-github-apps/authorizing-github-apps

### Billing and payments

- **GitHub Actions**: Learn how usage of GitHub Actions is measured against your free allowance and how to pay for additional use.
  - URL: https://docs.github.com/en/billing/concepts/product-billing/github-actions

- **GitHub Copilot licenses**: Learn how licenses for Copilot work, including usage measurement and managing your budget.
  - URL: https://docs.github.com/en/billing/concepts/product-billing/github-copilot-licenses

- **GitHub Models**: If you want to use GitHub Models beyond the free usage included in your account, you can choose to opt in to paid usage.
  - URL: https://docs.github.com/en/billing/concepts/product-billing/github-models

### Pull requests

- **About pull requests**: Pull requests let you propose, review, and merge code changes.
  - URL: https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/about-pull-requests

- **Creating a pull request**: Create a pull request to propose and collaborate on changes to a repository. These changes are proposed in a *branch*, which ensures that the default branch only contains finished and approved work.
  - URL: https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/creating-a-pull-request

- **Fork a repository**: A fork is a new repository that shares code and visibility settings with the original upstream repository.
  - URL: https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/working-with-forks/fork-a-repo

### More pages

- **Codespaces**: Create a codespace to start developing in a secure, configurable, and dedicated development environment that works how and where you want it to.
  - URL: https://docs.github.com/en/codespaces

- **Installation**: You can install GitHub Desktop on supported Windows or macOS operating systems.
  - URL: https://docs.github.com/en/desktop/installing-and-authenticating-to-github-desktop/installing-github-desktop

- **Prototype with AI models**: Find and experiment with AI models for free.
  - URL: https://docs.github.com/en/github-models/use-github-models/prototyping-with-ai-models

- **About Projects**: Projects is an adaptable, flexible tool for planning and tracking work on GitHub.
  - URL: https://docs.github.com/en/issues/planning-and-tracking-with-projects/learning-about-projects/about-projects

- **About GitHub Importer**: If your source code is stored on another Git-based hosting service, you can move the code to GitHub.com using GitHub Importer.
  - URL: https://docs.github.com/en/migrations/importing-source-code/using-github-importer/about-github-importer

- **OAuth app restrictions**: Organizations can choose which OAuth apps have access to their repositories and other resources by enabling OAuth app access restrictions.
  - URL: https://docs.github.com/en/organizations/managing-oauth-access-to-your-organizations-data/about-oauth-app-access-restrictions

- **Archive an organization**: You can archive an organization to make it read-only and indicate that it's no longer actively maintained. You can also unarchive organizations that have been archived.
  - URL: https://docs.github.com/en/organizations/managing-organization-settings/archiving-an-organization

- **Container registry**: You can store and manage Docker and OCI images in the Container registry.
  - URL: https://docs.github.com/en/packages/working-with-a-github-packages-registry/working-with-the-container-registry

- **Authenticating**: You can authenticate to the REST API to access more endpoints and have a higher rate limit.
  - URL: https://docs.github.com/en/rest/authentication/authenticating-to-the-rest-api

- **Permissions for fine-grained PATs**: For each permission granted to a fine-grained personal access token, these are the REST API endpoints that the app can use.
  - URL: https://docs.github.com/en/rest/authentication/permissions-required-for-fine-grained-personal-access-tokens

- **Code search syntax**: You can build search queries for the results you want with specialized code qualifiers, regular expressions, and boolean operations.
  - URL: https://docs.github.com/en/search-github/github-code-search/understanding-github-code-search-syntax

## How to Use This Skill

Reference these resources when working with GitHub.